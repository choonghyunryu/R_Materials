...
```{bash}
# split the header
head -n 1 diamond_1.csv > header

for i in `seq 2 $n`
do
  prev=`expr $i - 1`
  cat diamond_$prev.csv diamond_$prev.csv | grep -v carat > temp
  cat header temp > diamond_$i.csv
done
...
```{r, remove_temp, message=FALSE, warning=FALSE}
# 어떤 shell을 호출하는지 검색
